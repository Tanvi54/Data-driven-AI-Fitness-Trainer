...
FOODS = {
    ...
    'Dal': {'Calories': 150, 'Protein': 22, 'Carbs': 0, 'Fat': 2,'Type': 'Vegetarian'},
...
NUM_FOODS_TO_RECOMMEND_DINNER = 2
...
def generate_chromosome(user_info):
    ...
    if user_info['is_vegetarian']:
     ...
     # Avoid recommending foods that the user is allergic to
    for allergen in user_info['allergies']:
        for meal in MEAL_TYPES:
            for i, (food, info) in enumerate(FOODS.items()):
                if food == allergen:
                    chromosome[meal][i] = 0  # Avoid recommending allergenic foods
...
def fitness(chromosome, user_info):
    ...
    for meal in MEAL_TYPES:
        ...
        if user_info['age'] > 30:
         ...
         # Penalize the presence of allergenic foods
        for allergen in user_info['allergies']:
            if allergen in FOODS and chromosome[meal][list(FOODS.keys()).index(allergen)] == 1:
                meal_nutrition_score *= 0.5  # Example: Reduce score for allergenic foods    
...
def diet_genetic_algorithm(user_info):
    ...
    for generation in range(NUM_GENERATIONS):
        ...
        best_chromosome = population[fitness_scores.index(max(fitness_scores))]
         ...
         # Remove foods recommended for one meal from being recommended for others
        breakfast_recommendations = [i for i, food in enumerate(FOODS) if best_chromosome['breakfast'][i] == 1]
        lunch_recommendations = [i for i, food in enumerate(FOODS) if best_chromosome['lunch'][i] == 1]
        dinner_recommendations = [i for i, food in enumerate(FOODS) if best_chromosome['dinner'][i] == 1]

        for i in breakfast_recommendations:
            # Remove foods from lunch and dinner recommendations
            if i in lunch_recommendations:
                best_chromosome['lunch'][i] = 0
            if i in dinner_recommendations:
                best_chromosome['dinner'][i] = 0

        for i in lunch_recommendations:
            # Remove foods from breakfast and dinner recommendations
            if i in breakfast_recommendations:
                best_chromosome['breakfast'][i] = 0
            if i in dinner_recommendations:
                best_chromosome['dinner'][i] = 0

        for i in dinner_recommendations:
            # Remove foods from breakfast and lunch recommendations
            if i in breakfast_recommendations:
                best_chromosome['breakfast'][i] = 0
            if i in lunch_recommendations:
                best_chromosome['lunch'][i] = 0
